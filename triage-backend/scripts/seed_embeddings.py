...
Usage:
  .venv/bin/python scripts/seed_embeddings.py            # full ~6.5k sweep
...
def fetch_all_issues(github_token: str, max_issues: int, min_number: int = 0):
    ...
    Search API caps at 1000 results/query, so we page by state+year.
    """
        ...
        for year in range(2016, 2027):
            ...
            while page <= 10 and retries < 3:
                ...
                try:
                    ...
                    print(f"  Network error for {state}/{year}: {e}")
                    break
                ...
                print(f"  {state}/{year} page {page}: {len(all_issues)} total")
                if len(items) < 100:
                    break
                page += 1
