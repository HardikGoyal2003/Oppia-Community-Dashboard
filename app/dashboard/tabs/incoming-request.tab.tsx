...
import {
  resolveMemberAccessRequestAction,
} from "../dashboard.action";
import { MemberAccessRequestModel } from "@/lib/db/member-request-access.types";


export function IncomingRequestTab() {
  const [requests, setRequests] = useState<MemberAccessRequestModel[]>([]);
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error("Failed to load member access requests.");
        }
  ...
  const handleDecision = async (
    ...
    try {
      await resolveMemberAccessRequestAction({ email, decision });
      setRequests(prev =>
