    ...
    <div className="flex flex-wrap text-base sm:text-xl translate-y-px">
      {tabs.map((tab) => (
        <div
          ...
          className={`${tabBaseStyle} ${activeTab === tab ? "active-tab" : ""}`}
        >
          {tab === "team1" &&
            `${
      ...
      >
        <ArchiveIcon className="h-6 w-6 mr-1.5 mt-0.1.25" />{" "}
        {`Archived Issues (${categorizedProjectIssuesData?.archive.length ?? 0})`}
      </div>
    </div>
  );
};
